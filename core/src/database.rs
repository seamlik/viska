...
pub struct ProfileConfig {
    pub dir_data: PathBuf,
}

impl ProfileConfig {
    pub fn path_database(&self, account_id: &[u8]) -> std::io::Result<PathBuf> {
        let mut destination = self.dir_data.canonicalize()?;
    ...
    pub fn path_certificate(&self, account_id: &[u8]) -> std::io::Result<PathBuf> {
        let mut destination = self.dir_data.canonicalize()?;
        destination.push("account");
        destination.push(hex::encode_upper(account_id));
    ...
    pub fn path_key(&self, account_id: &[u8]) -> std::io::Result<PathBuf> {
        let mut destination = self.dir_data.canonicalize()?;
        destination.push("account");
        destination.push(hex::encode_upper(account_id));
...
/// * Async
#[riko::fun]
pub fn create_standard_profile(dir_data: PathBuf) -> Result<ByteBuf, CreateProfileError> {
    ...
    let path_certificate = profile_config.path_certificate(account_id.as_bytes())?;

    let path_account = path_certificate.parent().unwrap();
    log::debug!("Creating account directory {}", path_account.display());
    std::fs::create_dir_all(path_account)?;
    std::fs::write(&path_certificate, &bundle.certificate)?;
    std::fs::write(
        &profile_config.path_key(account_id.as_bytes())?,
        &bundle.key,
    )?;
    ...
    Database::create(&Storage::OnDisk(
        profile_config.path_database(account_id.as_bytes())?,
    ))?;
...
/// The generated account ID.
#[riko::fun]
pub fn create_mock_profile(dir_data: PathBuf) -> Result<ByteBuf, CreateProfileError> {
